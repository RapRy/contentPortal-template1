...
const Categories = () => {
  ...
  const toggleCheckmark = (check, i) => {
    ...
    if (data[i].subCatName === "All") {
      ...
      if (check === true) {
        ...
        let updatedFilters = filtered.filter(
          (fil) => fil !== data[i].subCatName
        );
  ...
  useEffect(() => {
    if (activeCat) {
      activeCat.subCategories.forEach((subCat) =>
        Object.assign(subCat, { isChecked: true })
      );
      const newData = [
        { _id: null, parent_id: null, subCatName: "All", isChecked: true },
  ...
  return (
    <div>
      <CategoriesWrap>
        {data.length > 0 &&
          data.map(({ subCatName, _id, isChecked }, i) => (
            <li key={_id}>
              <Checkbox>
                <CheckIcon isChecked={isChecked}>
                  {isChecked === true ? <span>✔</span> : ""}
                </CheckIcon>
                <input
                  type="checkbox"
                  id={subCatName}
                  checked={isChecked}
                  onChange={(e) => toggleCheckmark(isChecked, i)}
                />
              </Checkbox>
              <Label htmlFor={subCatName} isChecked={isChecked}>
                {subCatName}
              </Label>
            </li>
          ))}
...
const CheckIcon = styled.div`
  ...
  height: 15px;
  border: 2px solid
    ${({ isChecked }) => (isChecked === true ? "#7f1616" : "#ccc")};
  border-radius: 2px;
  position: absolute;
  top: 3px;
  left: 0;
